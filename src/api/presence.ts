...
export async function getOnSiteRoster(incidentId?: string): Promise<OnSitePerson[]> {
  ...
  const { data } = await client.get<OnSitePerson[]>('/presence/on-site-roster', { params });
  return data;
}
...
export async function markPersonSafe(
  ...
  await client.post('/presence/mark-safe', { personId, ...payload });
}

export async function getPresenceAuditLog(incidentId: string): Promise<{ entries: unknown[] }> {
  const { data } = await client.get<{ entries: unknown[] }>(`/presence/audit-log/${incidentId}`);
  return data;
}
